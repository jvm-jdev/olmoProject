const Demo24features4 = () => {
  return (
    <section id="features-4" className="pb-60 features-section division">
      <div className="container">
        {/* FEATURES-4 WRAPPER */}
        <div className="fbox-4-wrapper">
          <div className="row row-cols-1 row-cols-md-2">
            {/* FEATURE BOX #1 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="fbox-ico-center ico-65">
                    <span className="flaticon-dashboard" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">واجهة ودية</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
            {/* FEATURE BOX #2 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="fbox-ico-center ico-65">
                    <span className="flaticon-pantone" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">خيارات قوية</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
            {/* FEATURE BOX #3 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="fbox-ico-center ico-65">
                    <span className="flaticon-folder-3" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">مدير الملفات</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
            {/* FEATURE BOX #4 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="fbox-ico-center ico-65">
                    <span className="flaticon-resize" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">تحويل ملفات الوسائط</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
            {/* FEATURE BOX #5 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="fbox-ico-center ico-65">
                    <span className="flaticon-share" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">مشاركة الملفات</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
            {/* FEATURE BOX #6 */}
            <div className="col">
              <div className="fbox-4 pc-25 mb-40 wow fadeInUp">
                {/* Icon */}
                <div className="fbox-ico">
                  <div className="ico-65">
                    <span className="flaticon-layers" />
                  </div>
                </div>
                {/* Text */}
                <div className="fbox-txt">
                  {/* Title */}
                  <h5 className="h5-md">ملحقات وملحقات</h5>
                  {/* Text */}
                  <p className="p-lg">
                    {`العديد من برامح النشر المكتبي وبرامح تحرير صفحات الويب
                        تستخدم لوريم إيبسوم بشكل إفتراضي كنموذج عن النص، وإذا
                        قمت بإدخال "lorem ipsum" في أي محرك بحث ستظهر العديد من
                        المواقع الحديثة العهد في نتائج البحث.`}
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>{" "}
        {/* END FEATURES-4 WRAPPER */}
      </div>{" "}
      {/* End container */}
    </section>
  );
};

export default Demo24features4;
